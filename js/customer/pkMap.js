			...
			mui.each(r.data, function(j, k) {
				...
				var marker = new AMap.Marker({
					...
					"content": "<div class='experience-image'>" +
						"<span class='map-icon-url' data_id='" + k.StoreId + "' data_name='" + k.StoreName + "' data_status='" + k.Status + "' data_distance='" + k.distance + "' data_score='" + k.Score + "' style='position:absolute;border-radius:6px ;width:90px;height:20px;text-align: center;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;font-size:12px;color:#666;left:50%;bottom:50px;margin-left:-45px;border-radio:25%;background: #fff;'>" + k.StoreName + "</span>" +
					...
					"extData": {
						...
						"ImgUrl": k.ImgUrl,
						"Address": k.Address,
						"AddressStr": k.ProvinceName + k.CityName + k.DistrictName + k.Address,
					},
...
function gdMapMarkerClick(extData) {
	...
	if(mkid != undefined) { //防止点击当前位置
		...
		document.getElementById("lookFight").addEventListener("tap", function() {
			mui('#mainPopoverEl').popover('hide');
			mui.fire(plus.webview.getWebviewById('customer/war.html'), 'updateShop', {
				storeName: selectMaker.name,
				Address: selectMaker.Address,
				storeId: selectMaker.id,
				AddressStr: selectMaker.AddressStr
			})
			mui.back();
		})
	}
